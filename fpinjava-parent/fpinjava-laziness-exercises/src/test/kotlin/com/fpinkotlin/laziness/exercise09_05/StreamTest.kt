...
class StreamTest {
    ...
    fun testDropWhile() {
        ...
        val result = stream.dropWhile { it < 3 }
    ...
    fun testDropWhileEmpty() {
        assertEquals("[NIL]", empty<Int>().dropWhile { it < 4 }.toList().toString())
    }

    @Test
    fun testLongDropWhileTrue() {
        val stream = from(0)
        val result = stream.dropWhile { it < 200000 }.take(5)
    ...
    fun testLongDropWhileFalse() {
        val stream = from(0)
        val result = stream.dropWhile { it < 0 }.take(5)
