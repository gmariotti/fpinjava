...
interface Stream<A> {
    ...
    fun filter(p: (A) -> Boolean): Stream<A> {
        val stream = this.dropWhile { !p(it) }
        return if (stream.isEmpty) stream else cons(head = { stream.head }, tail = { stream.tail.filter(p) })
    }
...
fun from(i: Int): Stream<Int> = unfold(i) { Result.success(it to it + 1) }

fun <A> repeat(a: A): Stream<A> = iterate(a) { it }

fun <A> iterate(seed: A, f: (A) -> A): Stream<A> = cons(head = { seed }, tail = { iterate(f(seed), f) })

fun <A> iterate(seed: () -> A, f: (A) -> A): Stream<A> = cons(head = seed, tail = { iterate(f(seed()), f) })

fun fibs(): Stream<Int> = unfold(1 to 1) { Result.success(it.first to (it.second to (it.first + it.second))) }

fun <A, S> unfold(z: S, f: (S) -> Result<Pair<A, S>>): Stream<A> =
    f(z).map { (first, second) -> cons(head = { first }, tail = { unfold(second, f) }) }.getOrElse(empty())