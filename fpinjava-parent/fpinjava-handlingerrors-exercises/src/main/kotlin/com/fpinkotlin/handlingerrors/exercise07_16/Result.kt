...
import com.fpinkotlin.handlingerrors.exercise07_16.Result.Companion.empty
import com.fpinkotlin.handlingerrors.exercise07_16.Result.Companion.failure
import com.fpinkotlin.handlingerrors.exercise07_16.Result.Companion.success
...
interface Result<V> : Serializable {
	...
	fun getOrElse_(defaultValue: V): V {
		return getOrElseViaFoldLeft(defaultValue)
		//return getOrElseViaFoldRight(defaultValue);
	}

	fun getOrElseViaFoldLeft(defaultValue: V): V =
			foldLeft(defaultValue, { { v -> v } })

	fun getOrElseViaFoldRight(defaultValue: V): V =
			foldRight(defaultValue, { v -> { v } })
